...
//Instagram client id and client secret here, taken from .env
dotenv.load();
...
//implement FB app id and secret here, taken from .env
...
Facebook.api('oauth/access_token', {
  ...
  grant_type: 'client_credentials' //what is that??
}, function (res){
  if(!res|| res.error){
    console.log(!res ? 'error occured' : res.error);
    return;
  }
  var FACEBOOK_ACCESS_TOKEN = res.access_token;
})
    ...
    if (user) {
      ...
      Instagram.users.liked_by_self({
        ...
        complete: function(data) {
          ...
          //res.render('photos', {captions: captionArr});
        }
      }); 
    }
  });
